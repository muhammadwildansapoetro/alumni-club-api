...
export const updateUserProfileSchema = z.object({
  ...
  profile: z.object({
    fullName: z.string().min(1, "Nama lengkap wajib diisi").max(100, "Nama lengkap maksimal 100 karakter").optional(),
    ...
    companyName: z.string().max(100, "Nama perusahaan maksimal 100 karakter").optional(),
...
export const paginationSchema = z.object({
  ...
  limit: z.string().regex(/^\d+$/, "Limit harus berupa angka").transform(Number).default("10"),
});
...
export const alumniDirectoryQuerySchema = paginationSchema.extend({
  ...
  classYear: z.string().regex(/^\d+$/, "Class year harus berupa angka").transform(Number).optional(),
