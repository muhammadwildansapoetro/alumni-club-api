...
export const getAlumniDirectoryController = async (req: Request, res: Response) => {
  try {
    const page = parseInt(req.query.page as string) || 1;
    const limit = parseInt(req.query.limit as string) || 10;
    const department = req.query.department as string;
    const classYear = req.query.classYear ? parseInt(req.query.classYear as string) : undefined;
...
export const getUserByIdController = async (req: Request, res: Response) => {
  try {
    ...
    if (error instanceof Error && error.message === "Pengguna tidak ditemukan") {
...
export const updateUserProfileController = async (req: Request, res: Response) => {
  try {
    ...
    if (error instanceof Error && (
      ...
      error.message === "Akses ditolak - Anda hanya dapat memperbarui profil Anda sendiri"
    )) {
      return res.status(404).json({
        success: false,
        error: error.message,
...
export const updateUserPasswordController = async (req: Request, res: Response) => {
  try {
    ...
    const statusCode = error instanceof Error && (
      error.message === "Pengguna tidak ditemukan" ||
      error.message === "Akses ditolak - Anda hanya dapat memperbarui kata sandi Anda sendiri" ||
      error.message === "Kata sandi saat ini diperlukan" ||
      error.message === "Kata sandi saat ini salah" ||
      error.message === "Tidak dapat memperbarui kata sandi untuk pengguna Google"
    ) ? 400 : 500;
...
export const restoreUserController = async (req: Request, res: Response) => {
  try {
    ...
    if (error instanceof Error && error.message === "Pengguna yang dihapus tidak ditemukan") {
      return res.status(404).json({
        success: false,
        error: error.message,
      });
...
export const getDeletedUsersController = async (req: Request, res: Response) => {
  try {
    const page = parseInt(req.query.page as string) || 1;
    const limit = parseInt(req.query.limit as string) || 10;
...
export const createUserController = async (req: Request, res: Response) => {
  try {
    const { email, name, password, role, department, classYear, fullName } = req.body;
    ...
    if (isNaN(classYearNum) || classYearNum < 1960 || classYearNum > new Date().getFullYear()) {
      return res.status(400).json({
        success: false,
    ...
    const userRole: UserRole = role?.toUpperCase() === "ADMIN" ? "ADMIN" : "USER";
...
export const importAlumniFromCSVController = async (req: Request, res: Response) => {
  try {
    if (!req.file) {
      return res.status(400).json({
        success: false,
...
export const validateCSVTemplateController = async (req: Request, res: Response) => {
  try {
    if (!req.file) {
      return res.status(400).json({
        success: false,
